...
class _DashboardState extends State<Dashboard>  with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      scaffold: ScopedModel<DataModel>(model: appCtrl.getModel()!, child: ScopedModelDescendant<DataModel>(builder: (context, child, model) {
                  ...
                  builder: (_) {
                    return  DirectionalityRtl(
                      child: Scaffold(
                          ...
                          body: dashboardCtrl.bottomNavLists.isEmpty? Container(): DefaultTabController(
                              length: dashboardCtrl.bottomNavLists.length,
                              initialIndex: 0,
                              child: Stack(
                                children: [
                                  Scaffold(
                                      floatingActionButton: dashboardCtrl.tabController?.index ==
                                          0 || dashboardCtrl.tabController?.index == 1
                                          ? FloatingActionButton(
                                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(50)),

                                          backgroundColor: appCtrl.appTheme.primary,
                                          onPressed: ()=> dashboardCtrl.onTapActionButton(),
                                          child: SvgPicture.asset(
                                              dashboardCtrl.tabController?.index == 0 ? eSvgAssets
                                                  .message : eSvgAssets.mobile))
                                          : Container(),
                                      bottomNavigationBar: SizedBox(
                                          height: Sizes.s70,
                                          child: TabBar(
                                              onTap: (val) {
                                                dashboardCtrl.tabController?.index = val;
                                                dashboardCtrl.update();
                                              },
                                                    ...
                                                    icon: e.key == 3
                                                        ...
                                                        child: ClipRRect(
                                                            ...
                                                            child: dashboardCtrl
                                                                ...
                                                                dashboardCtrl.data!,
                                                                fit: BoxFit.cover)
                                                                : Text(appCtrl.user['name'][0],style: AppCss.manropeMedium14.textColor(appCtrl.appTheme.white),).alignment(Alignment.center).paddingOnly(top: 2))
                                                    )
